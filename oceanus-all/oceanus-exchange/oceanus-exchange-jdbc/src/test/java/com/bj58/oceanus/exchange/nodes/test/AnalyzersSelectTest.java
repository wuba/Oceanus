...
public class AnalyzersSelectTest {
	@Test
	...
	public void testSelectOrderBy() throws Exception{
		String sql = "select t.a as aa,t.b,c,d dd from table1 t join table2 t2 on t.a=t2.b join table3 t3 on t2.aa=t3.dd where a=1 and b in(2,3) and 1=1 and p=? AND name in (select name from users u where u!='hello' and 1=age) group by aa order by aa desc";
		SQLParser parser = new SQLParser();
		QueryTreeNode node = parser.parseStatement(sql);
		NodeAnalyzer<QueryTreeNode, AnalyzeResult> analyzer = Analyzers
				.get(node.getNodeType());
